...
module.exports = class {
  ...
  async run () {
    ...
    const statusArray = [
      (client) =>
client.user.setActivity(`//help | ExSaudia Kingdom | dsc.gg/exsaudia`, { type: "PLAYING" }),
      (client) => client.user.setActivity(`//help | ExSaudia Kingdom | dsc.gg/exsaudia`, { type: "PLAYING" })
    ];
    ...
    setInterval(() => {
      request("https://growtopiagame.com/detail", async (err, res, body) => {
        ...
        if (updatedWotd !== currentWotd.wotd_name && (Date.now() - currentWotd.lastUpdated) > 21600000 || !currentWotd.wotd_name || !currentWotd.lastUpdated) {
          ...
          const embed = new Discord.MessageEmbed()
            .setTitle("<:wotd:881767145988558909> World Of The Day <:wotd:881767145988558909>")
    ...
    this.client.on("message", async (message) => {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField("Item #1:", `${dailyQuest.item1.needed} **${dailyQuest.item1.name}**\n<:wls:881764640751763517>> ${dailyQuest.item1.quantity}/${dailyQuest.item1.price} WL${parseInt(dailyQuest.item1.price) > 1 ? "s" : ""}`)
        .addField("Item #2:", `${dailyQuest.item2.needed} **${dailyQuest.item2.name}**\n<:wls:881764640751763517> ${dailyQuest.item2.quantity}/${dailyQuest.item2.price} WL${parseInt(dailyQuest.item2.price) > 1 ? "s" : ""}`)
        .addField("Total", `<:wls:881764640751763517> ${parseInt(firstRawPrice) + parseInt(secondRawPrice)} WLs\nThis quest has been taken from [Jenulot's Tavern](https://discord.gg/jenulot).`)
        .setColor("GREEN")
        .setTimestamp();
