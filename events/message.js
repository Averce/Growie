...
module.exports = class {
  ...
  async run (message) {
    ...
    if (cmd.conf.args === true && !args.length) {
      return reply(`<:redTicks:881763434822254693> You haven't provided any argument.\nCorrect Usage: \`${guildSettings.prefix}${cmd.help.name} ${cmd.help.usage}\``);
    }
    ...
    if (message.author.id !== "414764511489294347" && message.author.id !== "") {
      if (!timestamps.has(message.author.id)) {
        ...
        if (now < expirationTime) {
          ...
          return reply(`<:redtick:911869301726773278> Slow it down dude. You have to wait ${timeLeft.toFixed(1)} seconds before using \`${cmd.help.name}\` again.`);
        }
    ...
    const noPermEmbed = new Discord.MessageEmbed()
      ...
      .setDescription(`
<:redtick:911869301726773278> Forbidden! You do not have the required permissions to use \`${cmd.help.name}\`.
